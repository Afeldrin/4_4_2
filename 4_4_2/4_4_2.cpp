...
#define SIZE 500	//!< размер массива провеки
...
int main()
{
	...
	std::cout << "Elapsed time (add 500 RBtree): " << elapsed_ms.count() << " mcs\n" << std::endl;
	...
	elapsed_ms = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);

	std::cout << "Elapsed time(add 500 vec): " << elapsed_ms.count() << " mcs\n" << std::endl;

	int b = vec[200];

	start_time = std::chrono::steady_clock::now();

	std::find(vec.begin(), vec.end(), b);

	end_time = std::chrono::steady_clock::now();

	elapsed_ms = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);

	std::cout << "Elapsed time (find vec): " << elapsed_ms.count() << " mcs\n" << std::endl;

	start_time = std::chrono::steady_clock::now();

	std::cout << tree.Find(300) << std::endl;

	end_time = std::chrono::steady_clock::now();

	elapsed_ms = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time);

	std::cout << "Elapsed time (find RBtree): " << elapsed_ms.count() << " mcs\n" << std::endl;
